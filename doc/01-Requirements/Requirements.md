...
User ──▶ Streamlit GUI ──▶ Controller
                   ...
                   │          └──▶ Transparency (kept/dropped reasons; transparency logs not persisted)
DocumentLoader ◀───┘
     ▲
...
| ING-06 | Ingestion UI messages may be shown ephemerally; ingestion does not persist transparency logs.            | Must     |   |
...
* **CH-03.10 (Durable history, not logs)** — **Persist** ConversationMemory state:
  ...
  – If persistence fails, continue in-memory and surface an **ephemeral** UI notice; determinism and bounded audit rules remain unchanged. **(Must)**
...
| RET-07 | Transparency view shows kept/dropped with reasons (ephemeral; no persisted transparency logs). | Must     |   |
...
| UI-03     | Transparency view of kept/dropped with reasons (ephemeral; no persisted transparency logs).                                                                                       | Must     |
...
(Reminder: transparency diagnostics are not persisted; ConversationMemory text log and Layer-E snapshot are persisted per CH-03.10.)&#x20;
...
| Privacy/Locality | Personal, single-user workflow; **no telemetry**; **ConversationMemory persists locally** (CH-03.10); **Debug Logger is optional per session**. |
| Extensibility    | Add a new agent or embedding model without touching > 1 file.                                                                                   |

(Adjusted to acknowledge ConversationMemory persistence and optional Debug Logger; does not reintroduce Tooling/CI/testing.)&#x20;
...
(Non-critical stack lines retained and trimmed to remain consistent with personal scope.)&#x20;
...
(Updated earlier to match the history/audit model; remains valid after Feature-1/2.)&#x20;
...
1. **Conversation Memory**
   ...
   • **Persistence:** after each turn, `conversation.log` contains both sides and is fsynced; on restart, Layer-G rebuilds from log tail; Layer-E loads from the last published snapshot (or backfills asynchronously).&#x20;
...
4. **UI**
   • Super-Prompt preview shows ❖ FILES and S\_ctx exactly; transparency view explains kept/dropped (transparency logs not persisted).
...
### Change Log (relative to v2.2)

**Feature-1 (Conversation History Persistence & Async Layer-E Embedding)**
• Added **CH-03.10 (Durable history, not logs)** to 4.2, specifying `conversation.log` append-only text log, Layer-G from log tail, separate `history_store.pkl` for Layer-E, and failure fallback.
• Added **UI-11 (Persist History toggle)** and **UI-12 (Clear History)** to 4.6.
• Non-Functional: updated Privacy/Locality row to acknowledge **local ConversationMemory persistence**.

**Feature-2 (Debug Logger)**
• Added new section **4.7 Debug Logger** with **LOG-01..LOG-08** (two files per session, vars toggle, truncation, levels, rotation, serialization, crash-survivability).

**Structural/Textual Adjustments (unavoidable for consistency)**
• Clarified mentions of “no persistent logs” to refer to transparency logs, while allowing ConversationMemory persistence (CH-03.10) and optional Debug Logger (4.7). No other sections were altered beyond these integrations.
